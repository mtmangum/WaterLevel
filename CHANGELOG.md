...
### Fixed
- **Chart left-alignment on mobile** — the "WATER LEVEL" title/legend row and the plot's y-axis gutter used the same fixed 44px inset as desktop, indenting the graph well past the stat grid's left edge (e.g. "OUTFLOW") and leaving unused width on the right. Title/legend row now aligns with the stat grid's own inset (12px) under 640px, and the y-axis gutter itself narrows to 30px on phone-width charts (derived from the chart's measured width) so more of the plot area is used.
